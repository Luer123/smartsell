...
<div class="smartsell-wrap">
    <div class="smartsell-header">
        <h1><?php esc_html_e('客户跟进日志', 'smartsell-assistant'); ?></h1>
        <p><?php esc_html_e('查看所有客户的跟进记录', 'smartsell-assistant'); ?></p>
    </div>
    ...
    <div class="smartsell-card">
        ...
        <div class="smartsell-filters">
            <div class="smartsell-filter-item">
                ...
                <input type="text" id="smartsell-customer-follow-person" class="smartsell-form-input" style="width: 150px;">
            </div>
            <div class="smartsell-filter-item">
                <label class="smartsell-filter-label"><?php esc_html_e('开始日期', 'smartsell-assistant'); ?></label>
                <input type="date" id="smartsell-customer-follow-start-date" class="smartsell-form-input" style="width: 150px;">
            </div>
            <div class="smartsell-filter-item">
                <label class="smartsell-filter-label"><?php esc_html_e('结束日期', 'smartsell-assistant'); ?></label>
                <input type="date" id="smartsell-customer-follow-end-date" class="smartsell-form-input" style="width: 150px;">
            </div>
            <div class="smartsell-filter-item">
                <button type="button" id="smartsell-customer-follow-filter" class="smartsell-btn smartsell-btn-primary">
        ...
        <table class="smartsell-table">
            ...
            <tbody id="smartsell-customer-follow-list">
                <tr>
        ...
        <!-- 分页 -->
        <div class="smartsell-pagination" data-type="customer-follow"></div>
    </div>
</div>
...
jQuery(document).ready(function($) {
    ...
    function loadFollowLogs(page) {
        var followPerson = $('#smartsell-customer-follow-person').val();
        var startDate = $('#smartsell-customer-follow-start-date').val();
        var endDate = $('#smartsell-customer-follow-end-date').val();
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('#smartsell-customer-follow-list').html('<tr><td colspan="6" class="smartsell-loading"><div class="smartsell-spinner"></div></td></tr>');
            },
            success: function(response) {
                if (response.success && response.data.code === 0) {
                    renderFollowLogs(response.data.data);
                } else {
                    $('#smartsell-customer-follow-list').html('<tr><td colspan="6" class="smartsell-empty"><?php esc_html_e('加载失败', 'smartsell-assistant'); ?></td></tr>');
                }
            }
        });
    }
    ...
    function renderFollowLogs(data) {
        ...
        if (!data.items || data.items.length === 0) {
            ...
            $.each(data.items, function(i, log) {
                ...
                html += '<tr>';
                html += '<td>' + escapeHtml(log.company || '-') + '</td>';
                html += '<td>' + escapeHtml(log.contact_person || '-') + '</td>';
        ...
        $('#smartsell-customer-follow-list').html(html);
    }
    
    // 筛选按钮点击
    $('#smartsell-customer-follow-filter').on('click', function() {
        loadFollowLogs(1);
    });
